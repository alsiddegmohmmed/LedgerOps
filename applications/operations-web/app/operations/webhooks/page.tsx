...
import { getMerchants, getTenant, getWebhookDeliveries, getWebhookEndpoints, type CoreMerchant, type CoreWebhookDelivery, type CoreWebhookEndpoint } from "../../../lib/core";
...
export default async function WebhooksPage() {
  ...
  return (
    <main><section className="shell">
      ...
      {visible.length === 0 ? <div className="panel status">No Merchant webhook endpoints are visible in your authorized scope.</div> : visible.map((item) => <MerchantWebhookSection key={item.merchant.merchantId} value={item} csrfToken={session.csrfToken} supportActive={supportActive} />)}
    </section></main>
  );
}
