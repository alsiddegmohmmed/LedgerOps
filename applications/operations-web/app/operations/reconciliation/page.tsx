...
export default async function ReconciliationPage() {
  ...
  return <main><section className="shell">
    ...
    <RunTable runs={runs.runs} currentRunIds={currentRunIds} displayLocale={displayLocale} displayTimezone={displayTimezone} />
  </section></main>;
}

function RunTable({ runs, currentRunIds, displayLocale, displayTimezone }: { runs: CoreReconciliationRun[]; currentRunIds: Set<string>; displayLocale: string; displayTimezone: string }) {
  return <section className="panel"><div className="eyebrow">Run history</div>{runs.length === 0 ? <p>No reconciliation runs have been created.</p> : <div className="table-wrap"><table><caption className="sr-only">Reconciliation run history</caption><thead><tr><th>Run</th><th>Batch</th><th>Status</th><th>Counts</th><th>Current</th><th>Open</th></tr></thead><tbody>{runs.map((run) => <tr key={run.runId}>
    ...
    <td><span className="monospace">{run.batchVersionId}</span><span className="table-secondary">cutoff {formatOperationsDateTime(run.sourceCutoff, displayLocale, displayTimezone)}</span></td>
