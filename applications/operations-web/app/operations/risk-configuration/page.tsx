...
import { getRiskConfiguration, getRiskConfigurationHistory, getTenant, getTenantConfiguration, type CoreRiskConfiguration } from "../../../lib/core";
...
export default async function RiskConfigurationPage() {
  ...
  return <main><section className="shell"><a href="/operations">← Operations</a><div className="eyebrow">Administration / Risk configuration</div><h1>{tenantResult.tenant.name}</h1><p>Risk configuration is Tenant-wide. Merchant-scoped authority cannot change it.</p>{currentResult.kind === "unavailable" || currentResult.kind === "error" ? <div className="panel status error">You cannot read Risk configuration for this Tenant.</div> : currentResult.kind === "ok" ? <RiskConfigurationForm configuration={currentResult.configuration} csrfToken={session.csrfToken} /> : null}<History result={historyResult.kind === "ok" ? historyResult.history : []} displayLocale={displayLocale} displayTimezone={displayTimezone} /></section></main>;
}

function History({ result, displayLocale, displayTimezone }: { result: CoreRiskConfiguration[]; displayLocale: string; displayTimezone: string }) { return <section className="panel"><div className="eyebrow">Immutable history</div>{result.length === 0 ? <p>No Risk configuration history is available.</p> : <div className="table-wrap"><table><caption className="sr-only">Risk configuration history</caption><thead><tr><th scope="col">Version</th><th scope="col">Review</th><th scope="col">Reject</th><th scope="col">Rules</th><th scope="col">Created</th></tr></thead><tbody>{result.map((item) => <tr key={`${item.profileId}-${item.version}`}><td>{item.version}</td><td>{item.reviewThreshold}</td><td>{item.rejectThreshold}</td><td>{item.rules.length}</td><td>{formatOperationsDateTime(item.createdAt, displayLocale, displayTimezone)}</td></tr>)}</tbody></table></div>}</section>; }
