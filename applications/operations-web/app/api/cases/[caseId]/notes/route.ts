import { addCaseNote } from "../../../../../lib/core";
import {
...
export async function POST(
  ...
  if (!body || !requiredUuid(caseId) || !note) return invalidCredentialActionRequest();
  const result = await addCaseNote(
    access.session.selectedTenantId!, caseId, access.session.accessToken, { note },
  );
  return mapCredentialActionResponse(result, 200, "case_note_failed");
}
