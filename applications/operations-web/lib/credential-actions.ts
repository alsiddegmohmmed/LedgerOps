...
export async function requireCredentialActionSession(request: Request): Promise<
...
export async function readCredentialActionBody(request: Request) {
  try {
...
export function requiredText(value: unknown, maximum: number) {
  if (typeof value !== "string") return null;
  const normalized = value.trim();
  return normalized && normalized.length <= maximum ? normalized : null;
}

export function invalidCredentialActionRequest() {
  return NextResponse.json({ type: "invalid_request" }, { status: 400 });
}

export function mapCredentialActionResponse(
  result: {
