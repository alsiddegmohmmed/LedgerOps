...
export async function requireWebhookActionSession(request: Request): Promise<
...
export async function readWebhookActionBody(request: Request) {
  try {
...
export function requiredWebhookUuid(value: unknown) {
  return typeof value === "string" && UUID.test(value) ? value : null;
}

export function requiredWebhookText(value: unknown, maximum: number) {
  if (typeof value !== "string") return null;
  const normalized = value.trim();
  return normalized && normalized.length <= maximum ? normalized : null;
}

export function validWebhookEventTypes(value: unknown): value is string[] {
  return Array.isArray(value)
    && value.length > 0
    && value.length <= EVENT_TYPES.size
    && value.every((eventType) => typeof eventType === "string" && EVENT_TYPES.has(eventType));
}

export function validWebhookPayload(value: unknown): value is Record<string, unknown> {
  return typeof value === "object" && value !== null && !Array.isArray(value);
}

export function invalidWebhookActionRequest() {
  return NextResponse.json({ type: "invalid_request" }, { status: 400 });
}

export function mapWebhookActionResponse(
  result: {
