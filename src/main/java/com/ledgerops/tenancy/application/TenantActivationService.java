...
import com.ledgerops.tenancy.api.TenantActivationPort;
...
import com.ledgerops.tenancy.domain.TenantActivationPrerequisites;
...
@Service("tenantLifecycleActivationService")
class TenantActivationService implements TenantActivationPort {
    ...
    @Override
    @Transactional
    public void lockForActivation(TenantReference tenant) {
        TenantId tenantId = TenantId.from(tenant.value());
    ...
    @Override
    @Transactional
    public TenantReference activate(TenantActivationRequest request) {
        ...
        Tenant changed;
        try {
            changed = current.activate(new TenantActivationPrerequisites(
                    request.initialTenantAdminActive(),
                    request.activeMerchantExists(),
                    request.onboardingConsistent()
            ));
        } catch (IllegalStateException exception) {
            throw new TenantLifecycleException(tenantId, TenantStatus.ACTIVE, exception);
        }
