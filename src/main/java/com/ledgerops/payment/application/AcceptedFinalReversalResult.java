...
public record AcceptedFinalReversalResult(
        UUID tenantId,
        UUID reversalId,
        UUID paymentId,
        UUID attemptId,
    ...
    public AcceptedFinalReversalResult {
        ...
        if (finalCategory != ProviderResultCategory.SUCCESS
                ...
                && finalCategory != ProviderResultCategory.TEMPORARY_FAILURE
                && finalCategory != ProviderResultCategory.PERMANENT_FAILURE) {
            throw new IllegalArgumentException("Accepted Reversal result must be final");
        }
